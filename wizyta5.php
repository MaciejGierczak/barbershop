            ...
            <div class="col-lg-8 col-xl-6">
                <div class="card rounded-3">
                    ...
                    <div class="card-body p-4 p-md-5">
                        ...
                        $_SESSION['service_id']=$_POST['service'];
                        $conn = new mysqli('localhost', 'root', '', 'barber');
                        $sql="Select  worker.id as wid,users_data.name as imie,users_data.surname as nazwi from schedule_terms inner join workers_schedule on worker_id=workers_schedule.worker_id inner join salon on workers_schedule.salon_id=salon.id inner join worker on worker.id=workers_schedule.worker_id inner join users on users.id=worker.users_id inner join users_data on users_data.users_id=users.id where status='free' and salon.id=".$_SESSION['salon']." and workers_schedule.date_of_shift='".$_SESSION['date']."' and workers_schedule_id=workers_schedule.id and time='".$_POST['time']."' order by time ASC";
                        $result=$conn->query($sql);
                        if($result-> num_rows>0) {
                        ?>
                        <form class="px-md-2" action="wizyta_add.php" method="post">
                            <div class="mb-4">
                                <div class="form-outline mb-4">
                                    <label class="form-label" for="form3Example1q">Wybierz pracownika</label>
                                    <select name="worker">
                                        <?php
                                        while ($row = $result->fetch_assoc()) {

                                            echo "<option value=" . "'" . $row['wid'] . "'" . ">" . $row['imie']." ".$row['nazwi']. "</option>";
                                        }
                                        ?>
                                    </select>
                                    <?php
