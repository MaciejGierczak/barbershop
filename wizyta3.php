            ...
            <div class="col-lg-8 col-xl-6">
                <div class="card rounded-3">
                    ...
                    <div class="card-body p-4 p-md-5">
                        ...
                        $sql="Select DISTINCT `time` from schedule_terms inner join workers_schedule on worker_id=workers_schedule.worker_id inner join salon on workers_schedule.salon_id=salon.id where status='free' and salon.id=".$_SESSION['salon']." and workers_schedule_id=workers_schedule.id and workers_schedule.date_of_shift='".$_SESSION['date']."' order by time ASC";
                        $result=$conn->query($sql);
                        if($result-> num_rows>0) {
                        ?>
                        <form class="px-md-2" action="wizyta5.php" method="post">
                            <div class="mb-4">
                                <div class="form-outline mb-4">
                                    <label class="form-label" for="form3Example1q">Dostępne Godziny </label>
                                    <select name="time">
                                        <?php
                                ...
                                ?>
                                    <div class="mb-4">
                                        <div class="form-outline mb-4">
                                            <label class="form-label" for="form3Example1q">Wybierz usługe</label>
                                            <select name="service">
                                                <?php
                                                while ($row = $result->fetch_assoc()) {

                                                    echo "<option value=" . "'" . $row['id'] . "'" . ">" . $row['name_of_service'] . "</option>";
                                                }
                                                ?>
                                            </select>
                                        </div>
